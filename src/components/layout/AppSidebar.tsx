...
export function AppSidebar() {
  ...
  return (
    <aside className={cn(
      ...
      {/* Logo */}
      <div className={cn('flex items-center h-16 border-b border-sidebar-border px-4', collapsed && 'justify-center px-0')}>
        <BrandLogo
          size="sm"
          showText={!collapsed}
      ...
      <nav className="flex-1 overflow-y-auto scrollbar-thin py-4 px-3 space-y-6">
        <div className="space-y-1">
          {!collapsed && <div className="px-3 mb-2 text-[10px] uppercase tracking-widest text-white/30 font-semibold">Principal</div>}
          {items.map(item => {
            ...
            return (
              ...
              >
                ...
                {!collapsed && isActive && <span className="ml-auto w-1.5 h-1.5 rounded-full bg-vayase-accent" />}
              </NavLink>
            );
          })}
        </div>
        ...
        <div className="space-y-1">
          {!collapsed && <div className="px-3 mb-2 text-[10px] uppercase tracking-widest text-white/30 font-semibold">Outils</div>}
          {secondary.map(item => {
